...
@pytest.mark.parametrize("line, winner", [
    ("5D 8C 9S JS AC 2C 5C 7D 8S QH", "Player 1"),
    ("5H 8C 3S 7S 10D 3C 6C 7D 5S QH", "Player 2"),
    ("5H 8C 3S 7S QD 3C 6C 7D 5S QH", "Player 1"),
])
def test_highest_number_wins(line, winner):
    assert play(line) == winner
...
def test_pair_wins(line, winner):
    assert play(line) == winner
...
def test_two_pair_wins(line, winner):
    assert play(line) == winner
...
def test_three_of_a_kind_wins(line, winner):
    assert play(line) == winner
...
def test_straight_wins(line, winner):
    assert play(line) == winner
